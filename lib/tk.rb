...
module Tk
  ...
  class << self
    ...
    # Poll interval for background work progress updates (milliseconds)
    # Default 16ms ≈ 60fps. Lower = more responsive, higher = less CPU overhead.
    # Example: Tk.background_work_poll_ms = 8  # 120fps for smooth animations
    def background_work_poll_ms
      @background_work_poll_ms ||= 16
    end

    def background_work_poll_ms=(ms)
      @background_work_poll_ms = ms.to_i
    end

    # Whether to drop intermediate progress values when worker yields faster
    # than UI can poll. Default true (keeps UI responsive). Set to false for
    # testing when you need all values.
    def background_work_drop_intermediate
      return @background_work_drop_intermediate if defined?(@background_work_drop_intermediate)
      @background_work_drop_intermediate = true
    end

    def background_work_drop_intermediate=(value)
      @background_work_drop_intermediate = !!value
    end

    # Background work mode: :ractor or :thread (or custom registered modes)
    # Default: :ractor on Ruby 4.x+ (stable Ractor API), :thread on Ruby 3.x
    def background_work_mode
      @background_work_mode ||= RUBY_VERSION.split('.').first.to_i >= 4 ? :ractor : :thread
    end

    def background_work_mode=(mode)
      @background_work_mode = mode
    end

    # Register a custom background work mode.
    # The implementation class must respond to the BackgroundWork interface:
    #   .new(data, worker: nil, &block), #on_progress, #on_done, #on_message,
    #   #send_message, #pause, #resume, #stop, #close, #start, #done?
    def register_background_mode(name, klass)
      TkRactorSupport.register_background_mode(name, klass)
    end

    # List registered background work modes
    def background_modes
      TkRactorSupport.background_modes.keys
    end
    ...
    #      .on_done { puts "Complete!" }
    #
    def background_work(data, mode: nil, worker: nil, name: nil, &block)
      mode ||= background_work_mode
      task = TkRactorSupport::BackgroundWork.new(data, mode: mode, worker: worker, &block)
    ...
    def register_background_task(task)
      background_tasks << task
      # Clean up finished tasks
      background_tasks.reject!(&:done?)
    end
  end

  # Debug signal support (SIGUSR1 dumps thread/ractor info)
  require_relative 'tk/debug_signal'
  include TkDebugSignal
  ...
  # Process all pending events (user input, timers, redraws, idle tasks).
  # Blocks until the event queue is empty. Pass true for idle-only.
  # See: https://www.tcl-lang.org/man/tcl8.6/TclCmd/update.htm
  ...
  # Process only idle callbacks (display updates, window layout calculations).
  # Does NOT process user input or timers. Use when you need UI
  # changes visible immediately without processing other events.
  # See: https://www.tcl-lang.org/man/tcl8.6/TclCmd/update.htm
