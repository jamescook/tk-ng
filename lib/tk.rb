...
module TkConfigMethod
  ...
  def __cget_core(slot)
    ...
    # :nocov: -- extension-only (tcllib/iwidgets) - no core Tk widgets use __numlistval_optkeys
  ...
  def __configinfo_core(slot = nil)
    if TkComm::GET_CONFIGINFO_AS_ARRAY
      if (slot &&
        ...
        # :nocov:
        # Legacy JAPANIZED_TK code path - handles font option aliases like
        # latinfont/asciifont/kanjifont that don't exist in modern Tk (8.6+).
        # These options returned 2-element alias entries on old Japanese Tk builds.
        ...
        if slot
          ...
          # :nocov:
          # Extension-only: __numstrval_optkeys is [] in base class.
          # Only overridden by extensions like BWidget::Dialog (buttonwidth).
          when /^(#{__numstrval_optkeys.join('|')})$/
            conf = tk_split_simplelist(tk_call_without_enc(*(__confinfo_cmd << "-#{slot}")), false, true)
          ...
          # :nocov: -- extension-only (tcllib/iwidgets)
          when /^(#{__numlistval_optkeys.join('|')})$/
            # conf = tk_split_simplelist(_fromUTF8(tk_call_without_enc(*(__confinfo_cmd << "-#{slot}"))))
            conf = tk_split_simplelist(tk_call_without_enc(*(__confinfo_cmd << "-#{slot}")), false, true)
