...
static Tcl_DriverClose2Proc		DriverClose2;
#else
...
static int		Unmount(const char *path, Tcl_Size numBytes);
 ...
 *
 * Tcl 9 changed the struct layout: closeProc and seekProc are now void*
 * placeholders. Use close2Proc and wideSeekProc instead.
 */
...
NormalizePath(Tcl_Interp *interp, Tcl_Obj *pathPtr, int nextCheckPoint) {
	...
	if (NULL != hPtr) {
	    ...
	    {
		Tcl_Size tempLen;
		Tcl_GetStringFromObj(normLinkPathPtr, &tempLen);
		nextCheckPoint = (int)tempLen;
	    }
	    /* TODO */
...
/*
 * Tcl 9 uses close2Proc with flags parameter; Tcl 8 uses closeProc without.
 */
#if TCL_MAJOR_VERSION >= 9
static int
DriverClose2(ClientData instanceData, Tcl_Interp *interp, int flags) {
    /* Close the channel */
    TrofsChannel *tcPtr = (TrofsChannel *)instanceData;
    if ((flags & (TCL_CLOSE_READ | TCL_CLOSE_WRITE)) != 0) {
        return EINVAL;  /* Half-close not supported */
    }
 ...
 */
static int
Unmount(const char *path, Tcl_Size numBytes) {
